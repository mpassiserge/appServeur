...
- **Une ligne de préparation article pour chaque article de gamme sur le BT créé**
...
### Double préparation sur le BT:
- **Articles d'opération**: Via `qyRecOpeArt` sur le BT (NumOT = 1)
- **Articles de gamme**: Via `qyRecArtGam` pour chaque gamme sur le BT (consolidés)
...
- **Cas 2**: Travail complexe avec tâches indépendantes - chaque tâche a ses propres articles spécifiques répartis sur des OT séparés
- **Cas 3**: Travail complexe traité comme un bloc unique - consolidation de TOUS les articles (opération + gammes) sur un seul BT
...
- Les **articles de gamme** représentent les ressources spécifiques de chaque tâche, mais comme les tâches ne sont pas individualisées, tous ces articles sont consolidés sur le BT principal

Cette approche permet d'avoir une vue complète de tous les articles nécessaires (généraux + spécifiques) tout en simplifiant l'exécution avec un seul bon de travail centralisé.