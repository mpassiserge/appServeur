<xsl:template name="mainta:field">
	...
	<!-- Ajout des paramètres manquants pour la gestion du blur -->
	...
	<!-- Variables nécessaires pour les autres templates -->
	<xsl:param name="TYP_"/>
	<xsl:param name="SRC_"/>
	<xsl:param name="INH_"/>
	<xsl:param name="INR_"/>
	...
	<xsl:choose>
		...
		<xsl:when test="$type = 'MEMOPRINT'">
			...
			<iframe width="100%" height="100%" marginwidth="0" marginheight="0" frameborder="0">
				...
				<xsl:attribute name="src"><xsl:value-of select="/documnet/Aliases/MOS_XMLDLL"/>GetRTF2HTML?ID1=<xsl:value-of select="$valeur"/>&amp;TABLE=<xsl:value-of select="$table"/>&amp;PARTIAL=<xsl:choose><xsl:when test="$partial = 'yes'">YES&amp;DIVID=DV<xsl:value-of select="$id"/><xsl:value-of select="$idmemo"/></xsl:when><xsl:otherwise>NO</xsl:otherwise></xsl:choose></xsl:attribute>
			</iframe>
		</xsl:when>
		<xsl:otherwise>
